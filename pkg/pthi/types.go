...
type TunnelClosureInfo struct {
	...
	ClosureReason         uint8
	// Note: 3 bytes padding after this struct in C for 4-byte alignment, handled by buf.Next(3) in parsing
}
...
type CIRATunnelLogEntry struct {
	...
	// Note: 3 bytes padding after RemoteAccessConnectionTrigger in C, handled by buf.Next(3) in parsing
	MpsHostname          AMTANSIString
	ProxyUsed            uint8
	ProxyName            AMTANSIString
	AuthenticationMethod uint8
	ConnectedInterface   uint8
	// Note: 3 bytes padding after ConnectedInterface in C for uint32 alignment, handled by buf.Next(3) in parsing
...
type CIRAFailedConnectionLogEntry struct {
	...
	MpsHostname          AMTANSIString
	AuthenticationMethod uint8
	// Structure layout: InterfaceData[0] (368B data + 308B pad) + InterfaceData[1] (368B data + 308B pad) = 1352B total
	...
	ConnectedInterface     uint8
	// Note: 3 bytes padding after ConnectedInterface in C, handled by buf.Next(3) in parsing
