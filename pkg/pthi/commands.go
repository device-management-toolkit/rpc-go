...
func (pthi Command) GetCiraLog() (response GetCiraLogResponse, err error) {
	...
	// Each InterfaceData is 676 bytes total (368 data + 308 padding)
	for i := 0; i < 2; i++ {
		...
		// Skip 1 byte padding after DhcpMode (uint8) for 4-byte alignment before IpAddress (uint32)
		buf2.Next(1)
		...
		// Skip 308 bytes padding after each InterfaceData (676 bytes total per entry)
		buf2.Next(308)
	}
	...
	for i := 0; i < MAX_CONNECTION_DETAILS; i++ {
		...
		// Skip 9 bytes padding between ConnectionDetail array entries
		// Each ConnectionDetail is 260 bytes data, with 9 bytes padding between entries
		if i < MAX_CONNECTION_DETAILS-1 {
			buf2.Next(9)
		}
	}
