...
func outputCiraLogText(w io.Writer, result pthi.GetCiraLogResponse) {
	...
	fields := []field{
		{"IsTunnelOpened(0-Closed, 1-Opened)", fmt.Sprint(result.CiraStatusSummary.IsTunnelOpened)},
		...
		{"LastConnectionStatus(MPS:0 - Connection established successfully, 1 - Failed to connect)",
			fmt.Sprint(result.CiraStatusSummary.LastConnectionStatus)},
		{"LastConnectionTimestamp",
			fmt.Sprintf("%d (%s)",
				result.CiraStatusSummary.LastConnectionTimestamp,
				formatTimestamp(result.CiraStatusSummary.LastConnectionTimestamp))},
		{"LastTunnelStatus(0 - Session opened and closed successfully., 1 - Session failed due to an error)",
			fmt.Sprint(result.CiraStatusSummary.LastTunnelStatus)},
...
func printTunnelLogEntry(w io.Writer, entry pthi.CIRATunnelLogEntry) {
	fields := []field{
		...
		{"AuthenticationMethod(MPS:1 - Mutual TLS, 2 - Username and password)", fmt.Sprint(entry.AuthenticationMethod)},
...
func printTunnelClosureInfo(w io.Writer, info pthi.TunnelClosureInfo) {
	fields := []field{
		...
		{"ClosedByMps(0-Closed by AMT, 1-Close by MPS)", fmt.Sprint(info.ClosedByMps)},
...
func getTlsFailureCodeString(code uint8) string {
