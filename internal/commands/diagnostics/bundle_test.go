...
func TestBundleCommand_DefaultCollectorsRecordsPartialWSMan(t *testing.T) {
	...
	cmd := BundleCmd{
		Output: outputFile,
		runWSMan: func(command *WSManGetCmd, _ *commands.Context) error {
			command.fetchFailures = 1

			return os.WriteFile(command.Output, []byte(`[{"class":"IPS_SecIOService","data":{"status":"fetch_failed"}}]`), 0o644)
		},
	}
	...
	require.NoError(t, json.Unmarshal(manifestData, &manifest))
	require.Len(t, manifest.Collectors, 3)
	assert.Equal(t, bundleCollectorResult{Name: "wsman", File: "wsman.json", Status: "partial", Error: "1 WSMAN class(es) failed to retrieve"}, manifest.Collectors[2])
}
