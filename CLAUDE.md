...
- **The `name:"..."` struct tag controls the CLI flag name; `kong:"required"` does not affect it.** Kong resolves flag names in this order: (1) `name:` tag if present, (2) kebab-cased Go field name as fallback. The `kong:"required"` directive only marks a flag as mandatory — it does not override the `name:` tag. Concrete example: `configure.CIRACmd.MPSAddress` is declared `` `help:"MPS Address" env:"MPS_ADDRESS" name:"mpsaddress" kong:"required"` `` and Kong exposes it as **`--mpsaddress`** (the `name:` tag wins). Similarly `MPSCert` with `name:"mpscert" kong:"required"` exposes as **`--mpscert`**. **The only authoritative source for a flag name is `go run ./cmd/rpc/main.go <command> --help`** — verify there before assuming kebab-case conversion applies.
